...
function requireDbcsData$1 () {
	...
	dbcsData$1 = {
	    ...
	    'gbk': {
	        type: '_dbcs',
	        table: function() { return require$$2$2.concat(require$$3$1) },
	    },
	    'xgbk': 'gbk',
	    'isoir58': 'gbk',
...
function requireLib$2 () {
	...
	(function (module) {
		...
		iconv.getCodec = function getCodec(encoding) {
		    if (!iconv.encodings)
		        iconv.encodings = requireEncodings$1(); // Lazy load all encoding definitions.
...
const VERSION = "e9cd69a928c46a0fa481e8e64c4fcdb2037ca4cd";
...
const SPARK_URL_PREFIX = `/_spark`;
const APP_ID = (process.env.GITHUB_RUNTIME_PERMANENT_NAME || process.env.CODESPACE_NAME)?.substring(0, 20);
app.all(`${SPARK_URL_PREFIX}/kv*`, async (req, res) => {
    const simplifiedUrl = req.url.replace(SPARK_URL_PREFIX, '');
    const route = `${GITHUB_API_URL}/runtime/${APP_ID}${simplifiedUrl}`;
    try {
        const request = {
            method: req.method,
            headers: {
...
app.post(`${SPARK_URL_PREFIX}/llm`, bodyParser.json(), async (req, res) => {
    const route = 'https://models.github.ai/inference/chat/completions';
    try {
        const request = {
            method: req.method,
            headers: {
        ...
        if (!response.ok) {
            ...
            res.status(response.status).send({ error: `Error fetching data from models '${error}'` });
            return;
        }
        const responseText = await response.text();
        res.contentType(response.headers.get('content-type') || 'application/json')
...
app.get(`${SPARK_URL_PREFIX}/user`, async (_req, res) => {
    const route = `${GITHUB_API_URL}/runtime/${APP_ID}/user`;
    try {
        const request = {
            method: 'GET',
            headers: {
...
app.post(`${SPARK_URL_PREFIX}/loaded`, bodyParser.json(), async (req, res) => {
    const route = `${GITHUB_API_URL}/runtime/${APP_ID}/loaded`;
    try {
        const request = {
            method: 'POST',
            headers: {
...
// This is a special endpoint for determining if the spark has been
// fully setup and is ready to be used.
// Checked by https://github.com/github/spark-loading
app.get(`${SPARK_URL_PREFIX}/status`, async (_, res) => {
    res.send({
        status: 'complete',
        APP_ID,
    });
});
const __dirname = require$$0$4.dirname(fileURLToPath(import.meta.url));
